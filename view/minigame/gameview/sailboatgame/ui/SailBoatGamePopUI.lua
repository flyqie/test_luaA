...
slot0.initCountUI = function(slot0)
	...
	slot1:SetEndEvent(function ()
		uv0._event:emit(SailBoatGameView.COUNT_DOWN)
	end)
end
...
slot0.initLeavelUI = function(slot0)
	...
	onButton(slot0._event, findTF(slot0.leaveUI, "ad/btnOk"), function ()
		uv0:resumeGame()
		uv0._event:emit(SailBoatGameView.LEVEL_GAME, true)
	end, SFX_CANCEL)
	onButton(slot0._event, findTF(slot0.leaveUI, "ad/btnCancel"), function ()
		uv0:resumeGame()
		uv0._event:emit(SailBoatGameView.LEVEL_GAME, false)
	end, SFX_CANCEL)
end
...
slot0.initPauseUI = function(slot0)
	...
	onButton(slot0._event, findTF(slot0.pauseUI, "ad/btnOk"), function ()
		uv0:resumeGame()
		uv0._event:emit(SailBoatGameView.PAUSE_GAME, false)
	end, SFX_CANCEL)
end
...
slot0.updateSettlementUI = function(slot0)
	...
	if slot4 < slot3 then
		slot0._event:emit(SailBoatGameView.STORE_SERVER, slot3)
	end
...
slot0.backPressed = function(slot0)
	...
	elseif isActive(slot0.leaveUI) then
		slot0:resumeGame()
		slot0._event:emit(SailBoatGameView.LEVEL_GAME, false)
	elseif not isActive(slot0.pauseUI) and not isActive(slot0.pauseUI) then
		slot0:popPauseUI()
		slot0._event:emit(SailBoatGameView.PAUSE_GAME, true)
	else
		slot0:resumeGame()
	end
end
