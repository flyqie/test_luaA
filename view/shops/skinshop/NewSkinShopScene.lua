...
slot0.UpdateCommodities = function(slot0, slot1, slot2, slot3)
	...
	for slot8, slot9 in ipairs(slot1) do
		slot10 = slot9:CanUseVoucherType()

		if slot0:IsType(slot0.skinPageID, slot9) and slot0:IsFilterType(slot0.defaultIndex, slot9) and slot0:IsSearchType(slot4, slot9) and slot0:IsCouponType(slot0.isFilterCoupon, slot9) and slot0:IsVoucherType(slot0.isFilterVoucher, slot10) then
			table.insert(slot0.displays, slot9)
		end

		slot0.canUseVoucherCache[slot9.id] = slot10
	end
	...
	for slot9, slot10 in ipairs(slot0.displays) do
		...
		if not (slot10.type == Goods.TYPE_ACTIVITY or slot10.type == Goods.TYPE_ACTIVITY_EXTRA) then
			slot12 = slot10:GetPrice()
		end

		slot5[slot10.id] = slot12
	end
	...
	slot6 = #slot0.displays <= 0

	setActive(slot0.emptyTr, slot6)
